...
- **화면**: 관리자 → TOP4 지표 (`/admin/top-rank-stats`)
- **API**: `GET /api/admin/top-rank-stats`, `/options`
- 캐릭터 x 무기 x (특성 | 아이템 | 전술스킬) 단위로,
  **TOP4(1~4위)** 와 **전체(1~8위)** 지표를 나란히 보여준다
  (게임수 / 평균획득점수 / 평균TK / 이득률·점수 / 손실률·점수 + TOP4율)
- **집계 테이블을 새로 만들지 않는다.** 기존 요약 테이블이 이미
  `character_id + weapon_type + 항목 + game_rank` 단위라, 순위 8행을 1행으로 접기만 하면 된다
  - 측정: 특성 502ms / 아이템 960ms(콜드) → 워밍 후 199ms, 캐릭터 필터 시 2ms
  - `GROUP BY` 는 **항목 우선** 순서가 인덱스 순서보다 3배 빠르다 (측정으로 확인)
